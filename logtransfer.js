...
class LokiClient {
  ...
  groupStreams(items) {
    ...
    for (const it of items) {
      ...
      const key = Object.keys(labelsObj).sort().map(k => `${k}=${labelsObj[k]}`).join('|');
  ...
  async flushNow() {
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        const errMsg = err.response ? `${err.response.status} ${err.response.statusText}` : (err.message || err);
        if (attempt === this.maxRetries) {
...
module.exports = { start: startForwarding };
